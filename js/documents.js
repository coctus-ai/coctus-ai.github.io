...
const CoctusDocuments = (() => {
  ...
  function safeName(title) {
    return (title || 'coctus-file').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'coctus-file';
  }
  ...
  function toJson(session) {
    return JSON.stringify({
      ...
      session,
      memoryFacts: CoctusMemory.getFacts(),
    }, null, 2);
  }
  ...
  function downloadCodeBlock(code, lang, filenameHint) {
    ...
    const name = filenameHint ? filenameHint : `coctus-snippet.${ext}`;
