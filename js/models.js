...
const CoctusModels = (() => {
  ...
  const PROVIDERS = { PUTER: 'puter', OPENROUTER: 'openrouter', CUSTOM: 'custom' };
  ...
  function setProvider(p) {
    if (p !== PROVIDERS.PUTER && p !== PROVIDERS.OPENROUTER && p !== PROVIDERS.CUSTOM) return;
    provider = p;
  }
  ...
  // Free OpenRouter/Puter models have a real capability ceiling — this is
  // the escape hatch: point Coctus at ANY OpenAI-compatible chat
  // completions endpoint (a paid Anthropic/OpenAI key via a compatible
  // proxy, a local llama.cpp/Ollama/LM Studio server, together.ai, Groq,
  // etc.) for meaningfully stronger output when it's actually needed.
  // Stored the same way as locally-entered OpenRouter keys: localStorage
  // only, never in the repo/page source.
  const CUSTOM_LS_KEY = 'coctus_custom_endpoint_v1';
  function getCustomConfig() {
    try {
      const raw = localStorage.getItem(CUSTOM_LS_KEY);
      const cfg = raw ? JSON.parse(raw) : {};
      return { url: cfg.url || '', key: cfg.key || '', model: cfg.model || '' };
    } catch { return { url: '', key: '', model: '' }; }
  }
  function setCustomConfig({ url, key, model }) {
    try { localStorage.setItem(CUSTOM_LS_KEY, JSON.stringify({ url: (url || '').trim(), key: (key || '').trim(), model: (model || '').trim() })); } catch {}
  }
  function customEndpointReady() { const c = getCustomConfig(); return !!(c.url && c.model); }

  async function chatCustom(messages, { model, onToken, temperature, signal, maxTokens, reasoningEffort, _attempt } = {}) {
    const attempt = _attempt || 0;
    if (signal && signal.cancelled) throw new CancelledError();
    const cfg = getCustomConfig();
    if (!cfg.url || !cfg.model) {
      throw new Error('No custom endpoint configured. Set the URL and model in Settings → Custom endpoint first.');
    }
    const useModel = model === 'custom' || !model ? cfg.model : model; // the model dropdown shows one synthetic "custom" entry when this provider is active
    const body = { model: useModel, messages, stream: !!onToken };
    if (typeof temperature === 'number') body.temperature = temperature;
    if (maxTokens) body.max_tokens = maxTokens;
    if (reasoningEffort) body.reasoning = { effort: reasoningEffort }; // harmless if the target server ignores unknown fields
    ...
    try {
      ...
      throw new Error(`Could not reach custom endpoint at ${cfg.url}: ${err.message}. If this is a local server, check it's actually running and that CORS is enabled for this page's origin.`);
    }
    if (signal && signal.cancelled) throw new CancelledError();
    ...
    if (!resp.ok) {
      ...
      if (!onToken && attempt === 0 && resp.status >= 500) {
        await new Promise(r => setTimeout(r, RETRY_DELAY_MS));
        if (signal && signal.cancelled) throw new CancelledError();
        return chatCustom(messages, { model, onToken, temperature, signal, maxTokens, reasoningEffort, _attempt: attempt + 1 });
      }
      throw new Error(`Custom endpoint error: ${detail}`);
    }
    ...
    return readOpenAISSEStream(resp, signal, onToken);
  }
  ...
  async function listModels() {
    if (provider === PROVIDERS.CUSTOM) {
      const cfg = getCustomConfig();
      return cfg.model ? [{ id: cfg.model, label: cfg.model, group: 'Custom endpoint' }] : [];
    }
  ...
  function isReady() {
    ...
    if (provider === PROVIDERS.CUSTOM) return customEndpointReady();
  ...
  async function chat(messages, opts = {}) {
    ...
    if (p === PROVIDERS.CUSTOM) return chatCustom(messages, opts);
   ...
   */
  // A model going missing mid-conversation (404/400) triggers an automatic
  // switch to another live free model rather than a hard failure — pushed
  // here so app.js can surface a one-time "switched from X to Y" toast
  // instead of the user just seeing a silent model change or a dead error.
  const modelFallbackNotices = [];
  function drainModelFallbackNotices() {
    const out = modelFallbackNotices.splice(0, modelFallbackNotices.length);
    return out;
  }

  async function pickFallbackModel(excludeModel) {
    try {
      const list = await listModelsOpenRouter();
      const candidate = list.find(m => m.id !== excludeModel);
      return candidate ? candidate.id : null;
    } catch (e) {
      return null; // listing itself is down — nothing sensible to fall back to
    }
  }

  /** Shared OpenAI-compatible SSE reader — "data: {...}\n\n" chunks ending in "data: [DONE]". Used by both chatOpenRouter and the custom-endpoint provider, since both speak the same protocol. */
  async function readOpenAISSEStream(resp, signal, onToken) {
  ...
  // ---------------- local free-tier request tracking ----------------
  // OpenRouter's own /api/v1/key endpoint tracks CREDIT usage in dollars —
  // free-model calls cost $0 and never move that number, so it can't tell
  // you how many of your 50-or-1000 daily free-model requests are left.
  // There is no documented endpoint that exposes that count directly.
  // This tracks it locally instead: every attempted (not just successful —
  // OpenRouter's own docs confirm failed attempts still consume the daily
  // quota) call to a `:free` model increments a counter that resets at UTC
  // midnight. This is honestly an approximation scoped to THIS app/browser
  // — if the same key is used elsewhere, that usage isn't visible here.
  const FREE_COUNT_LS_KEY = 'coctus_or_free_count_v1';
  function todayUTC() { return new Date().toISOString().slice(0, 10); }
  function getFreeRequestCount() {
    try {
      const raw = JSON.parse(localStorage.getItem(FREE_COUNT_LS_KEY) || '{}');
      return raw.date === todayUTC() ? (raw.count || 0) : 0;
    } catch { return 0; }
  }
  function bumpFreeRequestCount() {
    try {
      const today = todayUTC();
      const raw = JSON.parse(localStorage.getItem(FREE_COUNT_LS_KEY) || '{}');
      const count = raw.date === today ? (raw.count || 0) + 1 : 1;
      localStorage.setItem(FREE_COUNT_LS_KEY, JSON.stringify({ date: today, count }));
      return count;
    } catch { return null; }
  }

  let keyTierCache = null; // { isFreeTier: bool } — cached per page load, doesn't change mid-session
  async function getKeyTierInfo() {
    if (keyTierCache) return keyTierCache;
    if (!openRouterKeyPresent()) return null;
    try {
      const resp = await fetch('https://openrouter.ai/api/v1/key', { headers: openRouterHeaders(KeyPool.next()) });
      if (!resp.ok) return null;
      const json = await resp.json();
      keyTierCache = { isFreeTier: !!json?.data?.is_free_tier };
      return keyTierCache;
    } catch { return null; }
  }

  /** Best-effort daily free-request budget summary for display — not authoritative, see note above. */
  async function getFreeRequestBudget() {
    const tier = await getKeyTierInfo();
    const cap = tier ? (tier.isFreeTier ? 50 : 1000) : null;
    return { used: getFreeRequestCount(), cap };
  }

  async function chatOpenRouter(messages, { model, onToken, temperature, signal, maxTokens, reasoningEffort, _attempt, _fallbackDepth } = {}) {
    const attempt = _attempt || 0;
    if (signal && signal.cancelled) throw new CancelledError();
    ...
    let resp;
    if (String(model).endsWith(':free')) bumpFreeRequestCount();
    try {
    ...
    if (!resp.ok) {
      ...
      // A model that's gone (removed from OpenRouter's free list, deprecated,
      // typo'd id, etc.) surfaces as 404 or a 400 whose message names the
      // model — this is NOT a key/quota problem, so rotating keys would just
      // fail identically poolSize times. Free models genuinely rotate off
      // OpenRouter's catalogue without much notice (verified directly
      // against their live API), so this WILL happen in normal use, not
      // just as an edge case.
      const modelMissing = resp.status === 404 || (resp.status === 400 && /model/i.test(detail));
      ...
      if (canRotate) {
        return chatOpenRouter(messages, { model, onToken, temperature, signal, maxTokens, reasoningEffort, _attempt: attempt + 1, _fallbackDepth });
      }

      const fallbackDepth = _fallbackDepth || 0;
      if (modelMissing && fallbackDepth < 2) {
        const nextModel = await pickFallbackModel(model);
        if (nextModel) {
          modelFallbackNotices.push({ from: model, to: nextModel });
          return chatOpenRouter(messages, { model: nextModel, onToken, temperature, signal, maxTokens, reasoningEffort, _fallbackDepth: fallbackDepth + 1 });
        }
      }
  ...
  async function chatOpenRouterWithTools(messages, { model, tools, onToken, temperature, signal, maxTokens, reasoningEffort, _attempt, _fallbackDepth } = {}) {
    const attempt = _attempt || 0;
    if (signal && signal.cancelled) throw new CancelledError();
    if (noToolSupport.has(model)) {
      const text = await chatOpenRouter(messages, { model, onToken, temperature, signal, maxTokens, reasoningEffort });
    ...
    if (!resp.ok) {
      ...
      if (isRateLimit) KeyPool.cooldown(key, KEY_COOLDOWN_429_MS);
      if (isQuota) KeyPool.cooldown(key, KEY_COOLDOWN_402_MS);
      if (canRotate) return chatOpenRouterWithTools(messages, { model, tools, onToken, temperature, signal, maxTokens, reasoningEffort, _attempt: attempt + 1, _fallbackDepth });
      ...
      if (modelMissing && fallbackDepth < 2) {
        ...
        if (nextModel) {
          modelFallbackNotices.push({ from: model, to: nextModel });
          return chatOpenRouterWithTools(messages, { model: nextModel, tools, onToken, temperature, signal, maxTokens, reasoningEffort, _fallbackDepth: fallbackDepth + 1 });
        }
      }
      ...
      if (!onToken && !isRateLimit && !isQuota && attempt === 0 && resp.status >= 500) {
        await new Promise(r => setTimeout(r, RETRY_DELAY_MS));
        if (signal && signal.cancelled) throw new CancelledError();
        return chatOpenRouterWithTools(messages, { model, tools, onToken, temperature, signal, maxTokens, reasoningEffort, _attempt: attempt + 1, _fallbackDepth });
      }
      throw new Error(`OpenRouter error: ${detail}`);
    }
