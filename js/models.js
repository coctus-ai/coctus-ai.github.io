 ...
 * the rest of the app only ever call LocxyModels.chat()/listModels() and
...
const LocxyModels = (() => {
  ...
  function openRouterHeaders(key) {
    return {
      ...
      'HTTP-Referer': (typeof location !== 'undefined' && location.href) || 'https://locxy.local',
      'X-Title': 'Coctus AI',
    };
  }
  ...
  async function speak(text) {
    if (!ttsAvailable()) throw new Error('Text-to-speech is not available (Puter.js has not loaded, or this session is using OpenRouter which has no TTS endpoint here).');
    const clean = String(text || '').replace(/```[\s\S]*?```/g, ' code block omitted ').slice(0, 3000);
    if (!clean.trim()) throw new Error('Nothing to read aloud.');
    return puter.ai.txt2speech(clean);
  }
