...
const CoctusModels = (() => {
  ...
  // nothing to do with which CHAT provider is selected, but it does need
  // Puter.js loaded and (for a real voice) the user actually signed in.
  // OpenRouter itself is a pure chat-completions API with no TTS endpoint
  // at all, so relying on Puter alone left "Listen" broken whenever
  // someone was using OpenRouter without also being signed into Puter.
  // Fixed by adding a second, fully independent path: the browser's own
  // native speechSynthesis (Web Speech API) — no account, no network call,
  // works the same regardless of which chat provider is active. Puter is
  // tried first when available (usually a higher-quality voice); if it's
  // unavailable OR it actually fails at call time, this transparently
  // falls back to the browser voice instead of just erroring out.
  function puterTtsAvailable() {
    return !!(window.puter && puter.ai && puter.ai.txt2speech);
  }
