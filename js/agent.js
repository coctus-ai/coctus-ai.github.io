...
You'll get the result back and can call another tool or give your real answer. When you're done using tools (or don't need any), respond normally with your actual answer — do not mention "TOOL_CALL" or tool mechanics to the user, just use the results naturally.`;
  ...
  async function reactLoopNative(baseMessages, { model, temperature, signal, onToolCall, onToken, maxToolCalls, provider, reasoningEffort } = {}) {
    ...
    for (let i = 0; i < cap; i++) {
      ...
      let res;
      try {
        res = await CoctusModels.chatTool(messages, { model, temperature, signal, tools: isLast ? undefined : specs, onToken, provider, reasoningEffort });
      } catch (err) {
        if (err instanceof CoctusModels.CancelledError) throw err;
        ...
        return reactLoopText(messages, { model, temperature, signal, onToolCall, onToken, maxToolCalls: cap - i, provider, reasoningEffort });
      }
    ...
    const forced = await CoctusModels.chatTool(
      ...
      { model, temperature, signal, onToken, provider, reasoningEffort }
    );
    return forced.text;
  }
  ...
  async function reactLoopText(baseMessages, { model, temperature, webSearch, signal, onToolCall, onToken, maxToolCalls, provider, reasoningEffort } = {}) {
    let messages = baseMessages.slice();
    const cap = maxToolCalls || MAX_TOOL_CALLS_PER_TURN;
    for (let i = 0; i < cap; i++) {
      const isLast = i === cap - 1;
      const probe = await callModel(messages, { model, temperature, webSearch, signal, provider, reasoningEffort }, onToken, !isLast);
    ...
    return callModel(
      messages.concat([{ role: 'user', content: 'Tool budget for this turn is used up — give your best answer now using what you already have, with no more tool calls.' }]),
      { model, temperature, webSearch, signal, provider, reasoningEffort }, onToken, false
    );
  }
...
{"discussion": "1-2 sentences: your read on what's actually being asked and the approach you'll take", "needsPlan": boolean, "goal": "one line restating the objective", "steps": [{"title": "3-6 word step name", "detail": "one short sentence on what this step does", "criteria": "the concrete, checkable condition that makes this step DONE — not a vague description of activity"}]}
Rules:
...
- criteria must be genuinely checkable, not restated activity. Bad: "Research the topic" (that's the detail, not a completion condition). Good: "At least 2 independent sources confirm the claimed CVE's affected version range." Bad: "Write the code." Good: "The function runs without error against the 3 example inputs in the request." If a step has no meaningful pass/fail condition beyond "I did the thing," it's probably not a step that needs to exist — fold it into an adjacent one.
...
Be conservative — most answers are OK. Do not nitpick style or completeness that wasn't actually requested.`;
    try {
      ...
      ], { model, temperature: 0, signal, reasoningEffort: 'medium' }), signal);
      const line = raw.trim();
  ...
  /**
   * Whether a code block is safe to actually RUN as a correctness check,
   * vs. only syntax-checkable. The js_exec/py_exec sandboxes are Web
   * Workers with no Node built-ins, no DOM, no filesystem, and no network
   * beyond what the worker itself allows — code that legitimately needs
   * any of that would "fail" for reasons that have nothing to do with
   * whether it's actually correct. Running it anyway and auto-"fixing" it
   * based on that failure would actively corrupt otherwise-correct code,
   * which is worse than not checking at all. This stays deliberately
   * conservative — false negatives (skipping a runnable block) cost
   * nothing; false positives (mis-flagging a correct block) actively harm.
   */
  function isSandboxRunnable(code, lang) {
    const l = String(lang || '').toLowerCase();
    if (!['js', 'javascript', 'py', 'python'].includes(l)) return false;
    const envDependent = /\brequire\s*\(|\bimport\s+\w|\bfrom\s+\w+\s+import\b|document\.|window\.|localStorage|fetch\s*\(|\bprocess\.|\bfs\.|\bopen\s*\(|\binput\s*\(|\bos\.|\bsys\.argv/;
    if (envDependent.test(code)) return false;
    if (code.length > 4000) return false; // keep the check fast; huge blocks are usually full apps/files anyway, not verifiable snippets
    return true;
  }

  async function runCodeBlock(code, lang) {
    const l = String(lang || '').toLowerCase();
    const tool = (l === 'py' || l === 'python') ? 'py_exec' : 'js_exec';
    return CoctusTools.run(tool, { code });
  }
  ...
  async function checkAndFixCode(text, model, signal, onNote) {
    ...
    for (const block of blocks) {
      ...
      let code = block.code;
      let failureKind = 'syntax';

      // Syntax passed — if it's a safe, self-contained snippet, also
      // actually RUN it to catch real bugs (undefined vars, wrong logic,
      // exceptions), not just malformed syntax.
      if (result.ok && isSandboxRunnable(code, block.lang)) {
        try {
          const execResult = await runCodeBlock(code, block.lang);
          if (!execResult.ok) {
            result = { ok: false, message: execResult.error || 'runtime error' };
            failureKind = 'runtime';
          }
        } catch (err) { /* sandbox itself failing (e.g. Pyodide load) isn't the code's fault — skip */ }
      }
      if (result.ok) continue;

      // Up to 2 attempts: fix, re-check the fix itself (same check that
      // flagged it — syntax or actual execution), and if it's STILL
      // broken, feed the new error back in for a second try.
      for (let attempt = 0; attempt < 2; attempt++) {
        onNote && onNote(`Fixing a ${failureKind} issue in a ${block.lang || 'code'} block: ${result.message}`);
        try {
          const fixed = await CoctusModels.chat([
            { role: 'system', content: `You fix broken code. You will be given one code block that failed with this ${failureKind === 'runtime' ? 'RUNTIME error when actually executed' : 'syntax check error'}: "${result.message}". Return ONLY the corrected code, no explanation, no markdown fences.` },
            { role: 'user', content: code },
          ], { model, temperature: 0, signal });
          code = fixed.replace(/^```[a-zA-Z0-9_+-]*\n?/, '').replace(/```\s*$/, '').trim();
          result = CoctusValidate.check(code, block.lang);
          if (result.ok && failureKind === 'runtime') {
            const retryExec = await runCodeBlock(code, block.lang).catch(() => ({ ok: true }));
            if (!retryExec.ok) { result = { ok: false, message: retryExec.error || 'runtime error' }; }
          }
          if (result.ok) break;
        } catch (err) {
          if (err instanceof CoctusModels.CancelledError) throw err;
...
Reply with ONLY one line: either "OK" or "ISSUES: <one short, concrete, actionable sentence describing what to fix>".`;
    try {
      ...
      ], { model: criticModel, temperature: 0, signal, reasoningEffort: 'medium' }), signal);
      const line = raw.trim();
      const m = line.match(/^ISSUES:\s*(.+)/i);
      return m ? { ok: false, issue: m[1] } : { ok: true };
    } catch (err) {
   ...
   */
  async function run({ history, userText, userContent, model, execModel, execProvider, systemPreamble, agentic, webSearch, runToCompletion, tools, signal, criticModel, temperature, deepResearch, showThinking, reasoningEffort, callbacks = {} }) {
    ...
    async function streamOneCall(messages, extraOpts = {}) {
      const opts = { temperature, reasoningEffort, ...extraOpts };
      ...
      if (tools) {
        const full = await reactLoop(messages, {
          ...
          onToolCall: callbacks.onToolCall, onToken, reasoningEffort: opts.reasoningEffort,
        });
        accumulated += full;
        return full;
      }
    ...
    if (!agentic || isTrivial) {
      ...
      if (!plan.needsPlan || !plan.steps || !plan.steps.length) {
        ...
        const runStep = async (i, step) => {
          ...
          const doneCondition = step.criteria || step.detail || 'the step is genuinely complete';
...
End your output with exactly one line in this format (used internally, not shown verbatim to the user): CRITERIA_CHECK: met — <5-10 words why> — OR — CRITERIA_CHECK: not met — <what's actually still missing>. Be honest here — a false "met" defeats the entire point of having a checklist.`;
          const stepMessages = [
          ...
          try {
            note = tools
              ? await reactLoop(stepMessages, { model: runExecModel, temperature: 0.4, webSearch, signal, provider: execProvider, onToolCall: callbacks.onToolCall, onToken: stepToken, maxToolCalls: toolCallCap, reasoningEffort })
              : await CoctusModels.chat(stepMessages, { model: runExecModel, temperature: 0.4, webSearch, signal, onToken: stepToken, provider: execProvider, reasoningEffort });
          } catch (err) {
            if (err instanceof CoctusModels.CancelledError) throw err;
            note = `(step failed: ${err.message})`;
          }
          // Parse the self-reported checklist status out of the note rather
          // than trusting a bare "done" — a step that ran without throwing
          // isn't the same thing as a step that actually met its criterion.
          let criteriaMet = null; // true | false | null (couldn't parse — treated as inconclusive, not a silent pass)
          const checkMatch = note.match(/CRITERIA_CHECK:\s*(met|not met)\s*[—-]?\s*(.*)$/im);
          let criteriaReason = '';
          if (checkMatch) {
            criteriaMet = /^met$/i.test(checkMatch[1]);
            criteriaReason = checkMatch[2].trim();
            note = note.slice(0, checkMatch.index).trim(); // strip the marker line from what's stored/shown as the note
          }
          const entry = `Step ${i + 1} — ${step.title}: ${note}`;
          scratchpad.push(entry);
          allNotes.push(entry);
          callbacks.onStepDone && callbacks.onStepDone(i, note, { met: criteriaMet, reason: criteriaReason });
        };
...
${scratchpad.join('\n\n')}${thinkingBlock}`;
    ...
    if (!isTrivial && accumulated.trim() && spend()) {
      ...
      if (!verdict.ok) {
        ...
        ], { model: runExecModel, temperature: 0.3, signal, provider: execProvider, reasoningEffort });
   ...
   * one model doing everything. The role set adapts to the active persona
   * (e.g. Security Research gets Recon → PoC/Exploit-Dev → Reviewer instead
   * of the generic Researcher → Writer → Critic) since "what phase 1
   * should actually gather" genuinely differs by domain — a generic
   * "researcher" role told to work on a pentest report doesn't know it
   * should be thinking about attack surface and reproducible evidence
   * specifically, and a generic "writer" isn't primed to write findings in
   * report form. Reuses the same onPlan/onStepStart/onStepDone callbacks as
   * run() so the existing trace panel renders the collaboration with no
   * new UI plumbing.
   */
  const TEAM_ROLE_SETS = {
    security: [
      { title: 'Recon', detail: 'Maps attack surface and gathers concrete, verifiable findings (passive recon, tool output) for the target/request.', system: 'You are the RECON agent in a three-agent security team (Recon → PoC/Exploit-Dev → Reviewer). Your only job: gather and state the concrete attack-surface facts, recon findings, and constraints relevant to the request below, using tools (cert_transparency, dns_lookup, ip_geolocation, security_headers, web_search/web_fetch) where they genuinely help. Do NOT write the final deliverable — write a tight, factual recon brief for the next agent to build on.' },
      { title: 'PoC / Exploit-Dev', detail: 'Builds the actual deliverable — report, PoC script, or tool — from the recon brief.', system: null },
      { title: 'Reviewer', detail: 'Reviews the draft against the brief for accuracy, reproducibility, and completeness before one revision pass.', system: null },
    ],
    code: [
      { title: 'Spec', detail: 'Nails down concrete requirements, constraints, and edge cases before any code is written.', system: 'You are the SPEC agent in a three-agent build team (Spec → Build → Review). Your only job: turn the request below into a tight, concrete specification — inputs/outputs, edge cases, constraints, and what "done" looks like. Do NOT write the implementation — write the spec for the Build agent to implement against.' },
      { title: 'Build', detail: 'Implements the actual deliverable against the spec.', system: null },
      { title: 'Review', detail: 'Reviews the implementation against the spec for bugs and gaps before one revision pass.', system: null },
    ],
    default: [
      { title: 'Researcher', detail: 'Gathers grounded facts and context for the request, using tools where they help.', system: null },
      { title: 'Writer', detail: 'Drafts the actual deliverable from the researcher\'s findings.', system: null },
      { title: 'Critic', detail: 'Reviews the draft against the original request and flags concrete problems.', system: null },
    ],
  };

  async function runTeam({ history, userText, userContent, model, execModel, execProvider, systemPreamble, webSearch, tools, signal, personaId, reasoningEffort, callbacks = {} }) {
    signal = signal || { cancelled: false };
    ...
    const roleSet = TEAM_ROLE_SETS[personaId] || TEAM_ROLE_SETS.default;
    const roles = roleSet.map(r => ({ title: r.title, detail: r.detail }));
    callbacks.onPlan && callbacks.onPlan({ goal: userText, steps: roles });

    // ---- Phase 1 (Recon / Spec / Researcher) ----
    ...
    try {
      brief = tools
        ? await reactLoop(researcherMessages, { model, temperature: 0.3, webSearch, signal, onToolCall: callbacks.onToolCall, onToken: researchToken, maxToolCalls: MAX_TOOL_CALLS_PER_TURN, reasoningEffort })
        : await CoctusModels.chat(researcherMessages, { model, temperature: 0.3, webSearch, signal, onToken: researchToken, reasoningEffort });
    } catch (err) {
      if (err instanceof CoctusModels.CancelledError) throw err;
      brief = `(${roleSet[0].title.toLowerCase()} phase failed: ${err.message} — proceeding without it)`;
    }
    callbacks.onStepDone && callbacks.onStepDone(0, brief);

    // ---- Phase 2 (execution phase — hybrid runs use the execution model here) ----
    callbacks.onStepStart && callbacks.onStepStart(1);
    callbacks.onPhase && callbacks.onPhase('execute', { model: runExecModel, provider: execProvider || null });
    const writerSystem = `${baseSystem}
You are the ${roleSet[1].title.toUpperCase()} in a three-agent team (${roleSet[0].title} → ${roleSet[1].title} → ${roleSet[2].title}). The ${roleSet[0].title} agent has prepared the brief below — use it as grounding. Produce the actual, complete final deliverable the user asked for. Do not mention "the ${roleSet[0].title.toLowerCase()}", "the brief", or the team process — just deliver a well-organized, direct answer.
${roleSet[0].title} brief:
${brief}`;
    ...
    try {
      draft = await CoctusModels.chat(writerMessages, {
        model: runExecModel, temperature: 0.5, webSearch, signal, provider: execProvider, reasoningEffort,
    ...
    // ---- Phase 3 (review phase — always the primary model) ----
    callbacks.onStepStart && callbacks.onStepStart(2);
    callbacks.onPhase && callbacks.onPhase('review', { model });
    const critic = await verifyAnswer(userText, draft, [brief], model, signal);
    let final = draft;
    if (!critic.ok) {
      callbacks.onStepDone && callbacks.onStepDone(2, `Flagged: ${critic.issue} — ${roleSet[1].title} is revising.`);
      callbacks.onNote && callbacks.onNote(`${roleSet[2].title} flagged an issue — ${critic.issue}`);
      callbacks.onPhase && callbacks.onPhase('execute', { model: runExecModel, provider: execProvider || null });
      const revised = await CoctusModels.chat([
        { role: 'system', content: `${writerSystem}\n\nThe ${roleSet[2].title} agent reviewed your draft and flagged a concrete issue: "${critic.issue}". Rewrite the full deliverable, fixing that issue. Output only the corrected final version — no meta-commentary about the fix or the review process.` },
        ...history,
        { role: 'user', content: finalUserContent },
        { role: 'assistant', content: draft },
      ], { model: runExecModel, temperature: 0.4, signal, provider: execProvider, reasoningEffort });
      if (revised && revised.trim()) {
        final = revised;
