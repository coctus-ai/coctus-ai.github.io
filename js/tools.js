...
const LocxyTools = (() => {
  ...
  function writeFile(args) {
    ...
    const res = LocxyWorkspace.writeFile(path, content);
    if (!res.ok) return res;
    return { ok: true, result: `Wrote ${content.length} byte(s) to ${res.path}. Project now has ${LocxyWorkspace.fileCount()} file(s): ${LocxyWorkspace.listFiles().map(f => f.path).join(', ')}` };
  }
