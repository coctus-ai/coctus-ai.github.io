...
const CoctusTargets = (() => {
  ...
  // ---------------- recon diffing ----------------
  /**
   * Compares two saved scan snapshots on the same target by each item's
   * stable `key` (falls back to `label` for older snapshots saved before
   * keys existed). Returns what's new, what disappeared, and what's
   * unchanged — the actual point of re-scanning a target over time.
   */
  async function diffScans(targetId, scanIdA, scanIdB) {
    const t = await getTarget(targetId);
    if (!t) return { ok: false, error: 'target not found' };
    const scanA = t.scans.find(s => s.id === scanIdA);
    const scanB = t.scans.find(s => s.id === scanIdB);
    if (!scanA || !scanB) return { ok: false, error: 'one or both scans not found' };
    // Always diff older -> newer regardless of click order, so "added" means
    // genuinely new attack surface, not an artifact of selection order.
    const [older, newer] = scanA.createdAt <= scanB.createdAt ? [scanA, scanB] : [scanB, scanA];
    const keyOf = (item) => item.key || item.label;
    const olderKeys = new Map(older.items.map(i => [keyOf(i), i]));
    const newerKeys = new Map(newer.items.map(i => [keyOf(i), i]));
    const added = [...newerKeys.entries()].filter(([k]) => !olderKeys.has(k)).map(([, v]) => v);
    const removed = [...olderKeys.entries()].filter(([k]) => !newerKeys.has(k)).map(([, v]) => v);
    const unchanged = [...newerKeys.keys()].filter(k => olderKeys.has(k)).length;
    return { ok: true, older, newer, added, removed, unchanged };
  }
  ...
  return {
    ...
    addScanSnapshot, isInScope, buildContextBlock, diffScans,
  };
})();
