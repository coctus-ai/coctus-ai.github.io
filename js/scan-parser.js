...
const CoctusScanParser = (() => {
  ...
  function parseNmapXML(doc) {
    ...
    doc.querySelectorAll('host').forEach(host => {
      ...
      if (!ports.length) {
        items.push({ type: 'host', key: `${ip}:up`, label, detail: 'Host up, no open ports reported', severity: null, host: ip, raw: null });
        return;
      }
      ports.forEach(port => {
  ...
  function parseNucleiJSONL(text) {
    ...
    text.split('\n').map(l => l.trim()).filter(Boolean).forEach(line => {
      ...
      const matchedAt = obj['matched-at'] || obj.matched_at || host;
      items.push({
        type: 'nuclei_finding',
        key: `${obj['template-id'] || obj.template_id || '?'}|${matchedAt}`,
  ...
  function parseGenericJSON(data) {
    ...
    const items = arr.map((obj, i) => ({
      type: 'generic',
      key: obj.id || obj.key || `${obj.name || obj.title || 'item'}|${obj.host || obj.url || i}`,
