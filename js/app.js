...
(() => {
  const els = {
    ...
    reportMdBtn: document.getElementById('reportMdBtn'),
    reportDocxBtn: document.getElementById('reportDocxBtn'),
    ...
    scanHistoryCount: document.getElementById('scanHistoryCount'),
    scanHistoryList: document.getElementById('scanHistoryList'),
    diffScansBtn: document.getElementById('diffScansBtn'),
    ...
    conciseToggle: document.getElementById('conciseToggle'),
    ...
    freeRequestBudget: document.getElementById('freeRequestBudget'),
    ...
    customEndpointSaveBtn: document.getElementById('customEndpointSaveBtn'),
    customEndpointTestBtn: document.getElementById('customEndpointTestBtn'),
    customEndpointStatus: document.getElementById('customEndpointStatus'),
  ...
  const PERSONAS = {
    ...
    codeReview: {
      ...
      prompt: '\nYou are in Code Review mode: the user is asking you to REVIEW existing code, not write new code from scratch. If they pasted code or it\'s in an uploaded file/attachment, review exactly what\'s there — do not rewrite it into your own style unless asked. Structure every review the same way, in this order: (1) Correctness — actual bugs, off-by-ones, unhandled edge cases, race conditions; use js_exec/py_exec to actually verify a suspected bug rather than asserting it. (2) Security — injection, unsafe deserialization, secrets, auth/authz gaps, anything a pentester would flag; be concrete about exploitability, not generic. (3) Design/maintainability — only flag things that would actually bite someone later, not style nitpicks. For each issue: quote the exact problematic line(s), explain the concrete failure mode (not "this could be an issue" — say what breaks and when), and give the fix. If the code is genuinely solid, say so plainly instead of manufacturing issues to seem thorough — a short "no significant issues found, here\'s what\'s done well" is a valid and honest review.',
      tools: true, temp: 0.15,
    },
    security: {
  ...
  async function init() {
    ...
    const validProviders = [CoctusModels.PROVIDERS.OPENROUTER, CoctusModels.PROVIDERS.PUTER, CoctusModels.PROVIDERS.CUSTOM];
    ...
    els.conciseToggle.checked = !!prefs.concise;
    els.reasoningEffortSelect.value = prefs.reasoningEffort || '';
    { const cc = CoctusModels.getCustomConfig(); els.customUrlInput.value = cc.url; els.customKeyInput.value = cc.key; els.customModelInput.value = cc.model; }
  ...
  async function refreshFreeRequestBudget() {
    if (CoctusModels.getProvider() !== CoctusModels.PROVIDERS.OPENROUTER || !CoctusModels.openRouterKeyPresent()) {
      els.freeRequestBudget.classList.add('hidden');
      return;
    }
    const budget = await CoctusModels.getFreeRequestBudget();
    if (budget.cap === null) { els.freeRequestBudget.classList.add('hidden'); return; }
    const pct = budget.used / budget.cap;
    els.freeRequestBudget.textContent = `~${budget.used}/${budget.cap} free-model requests used today (tracked by this app/browser only — not authoritative if this key is used elsewhere).`;
    els.freeRequestBudget.className = 'hint key-status ' + (pct >= 0.9 ? 'error' : pct >= 0.6 ? 'warn' : 'ok');
    els.freeRequestBudget.classList.remove('hidden');
  }
  ...
  async function populateModels() {
    ...
    els.customEndpointSection.classList.toggle('hidden', provider !== CoctusModels.PROVIDERS.CUSTOM);
    refreshFreeRequestBudget();
    ...
    if (!models.length) {
      const opt = document.createElement('option');
      opt.textContent = provider === CoctusModels.PROVIDERS.OPENROUTER ? 'No free models found right now on OpenRouter'
        : provider === CoctusModels.PROVIDERS.CUSTOM ? 'Set a model name in Settings → Custom endpoint'
        : 'No models available';
      els.modelSelect.appendChild(opt);
      return;
    }
  ...
  function createTrace(bodyEl, contentEl) {
    ...
    head.innerHTML = `<span class="trace-phase-dot"></span><span class="trace-label">Working…</span><span class="trace-badges"></span><span class="trace-time"></span><span class="chev">▸</span>`;
    const body = document.createElement('div');
    ...
    function updateBadges() {
      const badgesEl = head.querySelector('.trace-badges');
      const parts = [];
      if (stepCount) parts.push(`<span class="trace-badge">${doneCount}/${stepCount} steps</span>`);
      const fileCount = CoctusWorkspace.listFiles().length;
      if (fileCount) parts.push(`<span class="trace-badge">${fileCount} file${fileCount === 1 ? '' : 's'}</span>`);
    ...
    function showPlan(plan) {
      ...
      (plan.steps || []).forEach((step, i) => {
        ...
        el.dataset.step = i;
        el.innerHTML = `<span class="trace-step-marker"></span><div class="trace-step-body"><b>${escapeHtml(step.title)}</b><span class="trace-step-detail">${escapeHtml(step.detail || '')}</span>${step.criteria ? `<span class="trace-step-criteria">✓ ${escapeHtml(step.criteria)}</span>` : ''}<div class="trace-step-note"></div></div>`;
        planEl.appendChild(el);
      });
    }
    function markStep(i, status, criteriaCheck) {
      const el = planEl && planEl.querySelector(`.trace-step[data-step="${i}"]`);
      if (!el) return;
      el.classList.remove('active', 'done', 'criteria-met', 'criteria-unmet');
      if (status) el.classList.add(status);
      if (status === 'done') {
        doneCount++;
        updateBadges();
        if (criteriaCheck && criteriaCheck.met !== null) {
          el.classList.add(criteriaCheck.met ? 'criteria-met' : 'criteria-unmet');
          const criteriaEl = el.querySelector('.trace-step-criteria');
          if (criteriaEl) criteriaEl.textContent = (criteriaCheck.met ? '✓ ' : '✗ ') + (criteriaCheck.reason || (criteriaCheck.met ? 'Criterion met' : 'Criterion not met'));
        }
      }
    }
    ...
    function toolCall(call, result) {
      ...
      const isFileWrite = call.tool === 'write_file';
      const path = isFileWrite && call.args ? call.args.path : null;
      if (!result) {
        const label = isFileWrite && path ? `📝 Editing file: ${path}…` : `${icon} Calling ${call.tool}(${argStr})…`;
        pendingToolRow = addRow(label, 'tool-call tool-pending');
      } else {
        const row = pendingToolRow || addRow('', 'tool-call');
        row.className = 'trace-row tool-call ' + (result.ok ? 'tool-ok' : 'tool-err');
        if (isFileWrite && path && result.ok) {
          row.textContent = `📝 Wrote ${path}`;
        } else {
  ...
  async function runGeneration(fullUserText, userContent, recallText) {
    ...
    // Code and Code Review benefit the most from extra reasoning effort —
    // that's exactly where "the model missed an edge case" shows up — so
    // they default to High unless the user has explicitly picked a level
    // for themselves (an explicit choice always wins).
    const userReasoningEffort = CoctusMemory.getPrefs().reasoningEffort;
    const effectiveReasoningEffort = userReasoningEffort
      || (['code', 'codeReview'].includes(CoctusMemory.getPrefs().persona) ? 'high' : undefined);
    let systemPreamble = `You are Coctus AI, an agentic assistant embedded in a single-page web app. You are direct, precise, and helpful across coding, research, writing, and general reasoning. Format responses in clean markdown; use fenced code blocks with a language tag for any code. When a task has multiple parts (e.g. several files, a multi-section document), write all of them out in full — never summarize or promise to add the rest "later". Skip throat-clearing openers ("Great question!", "Sure, I'd be happy to...") — start with the actual content.`;
    if (CoctusMemory.getPrefs().concise) {
      systemPreamble += `\n\nConcise mode is ON: give the essential answer only. No preamble, no restating the question, no hedging ("it's worth noting that...", "keep in mind..."), no closing summary of what you just said, no "let me know if you'd like more detail" — if more detail is worth having, just include it concisely instead of offering to add it later. This means SHORTER, not less complete or less rigorous — a genuinely multi-part deliverable (multiple files, a full report) still gets everything it needs, just with zero padding around it.`;
    }
    ...
    const callbacks = {
      ...
      onStepDone: (i, note, criteriaCheck) => { ensureMessage(); trace.markStep(i, 'done', criteriaCheck); },
    ...
    try {
      const final = teamMode
        ? await CoctusAgent.runTeam({ history, userText: fullUserText, userContent, model, execModel, execProvider, systemPreamble, webSearch: liveSearch, tools, signal: state.signal, personaId: CoctusMemory.getPrefs().persona, reasoningEffort: effectiveReasoningEffort, callbacks })
        : await CoctusAgent.run({ history, userText: fullUserText, userContent, model, execModel, execProvider, systemPreamble, agentic, webSearch: liveSearch, runToCompletion, tools, criticModel, temperature: persona.temp, deepResearch, showThinking, reasoningEffort: effectiveReasoningEffort, signal: state.signal, callbacks });
      ...
      refreshAuthStatus();
      refreshFreeRequestBudget();
      if (typeof CoctusModels !== 'undefined' && CoctusModels.drainModelFallbackNotices) {
        const notices = CoctusModels.drainModelFallbackNotices();
        const seen = new Set();
        notices.forEach(n => {
          const key = `${n.from}→${n.to}`;
          if (seen.has(key)) return; // same swap can legitimately fire more than once per turn (plan + steps + verifier); one toast is enough
          seen.add(key);
          showToast(`"${n.from}" appears to be unavailable — switched to "${n.to}" for this response.`, 'warn', 6000);
        });
      }
      refreshKeyPoolStatus();
    }
  ...
  function openFindingEditor(finding) {
    ...
    els.findingEditor.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
  ...
  function renderScanPreview(result) {
    ...
    els.scanPreview.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
  }

  function renderScanHistory(t) {
    els.scanHistoryCount.textContent = t.scans.length || '';
    els.scanHistoryList.innerHTML = '';
    els.diffResults.classList.add('hidden');
    if (!t.scans.length) {
      els.scanHistoryList.innerHTML = '<p class="memory-empty">No scans imported yet — parse one below to start building history.</p>';
      return;
    }
    [...t.scans].sort((a, b) => b.createdAt - a.createdAt).forEach(scan => {
      const item = document.createElement('label'); item.className = 'knowledge-item scan-history-item';
      const cb = document.createElement('input'); cb.type = 'checkbox'; cb.dataset.scanId = scan.id;
      cb.addEventListener('change', () => {
        const checked = els.scanHistoryList.querySelectorAll('input[type=checkbox]:checked');
        if (checked.length > 2) cb.checked = false; // cap at 2 — diffing is pairwise
      });
      const info = document.createElement('div'); info.className = 'knowledge-item-info';
      const name = document.createElement('span'); name.className = 'knowledge-item-name'; name.textContent = `${scan.tool} — ${scan.label}`;
      const meta = document.createElement('span'); meta.className = 'knowledge-item-meta';
      meta.textContent = `${scan.items.length} item(s) · ${fmtDate(scan.createdAt)}`;
      info.appendChild(name); info.appendChild(meta);
      item.appendChild(cb); item.appendChild(info);
      els.scanHistoryList.appendChild(item);
    });
  }

  function renderDiffResults(diff) {
    const { added, removed, unchanged, older, newer } = diff;
    const sevBadge = (item) => `<span class="finding-severity-badge" style="background:${SEVERITY_COLOR[item.severity] || '#9CA3AF'}">${item.severity || '—'}</span>`;
    const row = (item, cls) => `<div class="diff-row ${cls}">${sevBadge(item)}<span class="diff-row-label">${item.label}</span></div>`;
    els.diffResults.innerHTML = `
      <p class="hint"><strong>${older.label}</strong> (${fmtDate(older.createdAt)}) → <strong>${newer.label}</strong> (${fmtDate(newer.createdAt)}) · ${unchanged} unchanged</p>
      <div class="diff-col-header diff-added">+ New (${added.length})</div>
      ${added.length ? added.map(i => row(i, 'diff-added')).join('') : '<p class="memory-empty">Nothing new.</p>'}
      <div class="diff-col-header diff-removed">− Gone (${removed.length})</div>
      ${removed.length ? removed.map(i => row(i, 'diff-removed')).join('') : '<p class="memory-empty">Nothing disappeared.</p>'}
    `;
    els.diffResults.classList.remove('hidden');
    els.diffResults.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
  }
  ...
  function bindEvents() {
    ...
    els.conciseToggle.addEventListener('change', () => CoctusMemory.savePrefs({ ...CoctusMemory.getPrefs(), concise: els.conciseToggle.checked }));
    els.reasoningEffortSelect.addEventListener('change', () => CoctusMemory.savePrefs({ ...CoctusMemory.getPrefs(), reasoningEffort: els.reasoningEffortSelect.value }));
    ...
    els.customEndpointSaveBtn.addEventListener('click', async () => {
      const url = els.customUrlInput.value.trim();
      const modelName = els.customModelInput.value.trim();
      if (!url || !modelName) { showToast('URL and model name are both required.', 'warn', 3500); return; }
      CoctusModels.setCustomConfig({ url, key: els.customKeyInput.value.trim(), model: modelName });
      showToast('Custom endpoint saved.');
      await populateModels();
    });
    els.customEndpointTestBtn.addEventListener('click', async () => {
      const url = els.customUrlInput.value.trim();
      const modelName = els.customModelInput.value.trim();
      if (!url || !modelName) { els.customEndpointStatus.textContent = 'Enter URL and model name first.'; els.customEndpointStatus.className = 'hint key-status warn'; return; }
      CoctusModels.setCustomConfig({ url, key: els.customKeyInput.value.trim(), model: modelName });
      els.customEndpointTestBtn.disabled = true;
      els.customEndpointStatus.textContent = 'Testing…';
      els.customEndpointStatus.className = 'hint key-status';
      try {
        const reply = await CoctusModels.chat(
          [{ role: 'user', content: 'Reply with exactly one word to confirm connectivity.' }],
          { model: modelName, provider: CoctusModels.PROVIDERS.CUSTOM }
        );
        els.customEndpointStatus.textContent = `✓ Reachable — replied: ${String(reply).slice(0, 80)}`;
        els.customEndpointStatus.className = 'hint key-status ok';
        await populateModels();
      } catch (err) {
        els.customEndpointStatus.textContent = `✗ ${err.message}`;
        els.customEndpointStatus.className = 'hint key-status error';
      }
      els.customEndpointTestBtn.disabled = false;
    });
    ...
    async function currentTargetForReport() {
      if (!currentTargetId) { showToast('Open a target first.', 'warn', 3000); return null; }
      const t = await CoctusTargets.getTarget(currentTargetId);
      if (!t.findings.length) { showToast('No findings to report yet — add or import some first.', 'warn', 3500); return null; }
      return t;
    }
    ...
    els.reportMdBtn.addEventListener('click', async () => { const t = await currentTargetForReport(); if (t) withButtonBusy(els.reportMdBtn, 'Generating…', async () => CoctusReportBuilder.exportMarkdown(t)); });
    els.reportDocxBtn.addEventListener('click', async () => { const t = await currentTargetForReport(); if (t) withButtonBusy(els.reportDocxBtn, 'Generating…', async () => CoctusReportBuilder.exportDocx(t)); });
    els.reportPdfBtn.addEventListener('click', async () => { const t = await currentTargetForReport(); if (t) withButtonBusy(els.reportPdfBtn, 'Generating…', async () => CoctusReportBuilder.exportPdf(t)); });
    ...
    els.scanImportBtn.addEventListener('click', async () => {
      ...
      const t2 = await CoctusTargets.getTarget(currentTargetId);
      renderFindingsList(t2); renderTargetsList(); renderScanHistory(t2);
    ...
    els.scanSaveSnapshotBtn.addEventListener('click', async () => {
      ...
      const t3 = await CoctusTargets.getTarget(currentTargetId);
      renderScanHistory(t3);
      els.scanPreview.classList.add('hidden');
      els.scanPasteInput.value = '';
    });
    els.diffScansBtn.addEventListener('click', async () => {
      if (!currentTargetId) return;
      const ids = [...els.scanHistoryList.querySelectorAll('input[type=checkbox]:checked')].map(cb => cb.dataset.scanId);
      if (ids.length !== 2) { showToast('Select exactly 2 scans to compare.', 'warn', 3000); return; }
      const diff = await CoctusTargets.diffScans(currentTargetId, ids[0], ids[1]);
      if (!diff.ok) { showToast(diff.error, 'warn', 4000); return; }
