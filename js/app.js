...
(() => {
  ...
  async function runGeneration(fullUserText, userContent, recallText) {
    ...
    try {
      const final = teamMode
        ? await CoctusAgent.runTeam({ history, userText: fullUserText, userContent, model, execModel, execProvider, systemPreamble, webSearch: liveSearch, tools, signal: state.signal, personaId: CoctusMemory.getPrefs().persona, reasoningEffort: CoctusMemory.getPrefs().reasoningEffort || undefined, callbacks })
        : await CoctusAgent.run({ history, userText: fullUserText, userContent, model, execModel, execProvider, systemPreamble, agentic, webSearch: liveSearch, runToCompletion, tools, criticModel, temperature: persona.temp, deepResearch, showThinking, reasoningEffort: CoctusMemory.getPrefs().reasoningEffort || undefined, signal: state.signal, callbacks });
  ...
  function bindEvents() {
    ...
    els.reportMdBtn.addEventListener('click', async () => { const t = await currentTargetForReport(); if (t) CoctusReportBuilder.exportMarkdown(t); });
    els.reportDocxBtn.addEventListener('click', async () => { const t = await currentTargetForReport(); if (t) CoctusReportBuilder.exportDocx(t); });
    els.reportPdfBtn.addEventListener('click', async () => { const t = await currentTargetForReport(); if (t) CoctusReportBuilder.exportPdf(t); });
